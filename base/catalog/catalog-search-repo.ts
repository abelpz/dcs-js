...
export interface catalogSearchRepoParams extends baseParams {
 ...
 */
export default function catalogSearchRepo({
  owner,
  repo,
  q,
  owner,
  ...
  auth,
  options,
}: catalogSearchRepoParams): Promise<CatalogSearchResults> {
  const requestParams = {
    method: "GET",
    basePath: options?.basePath,
    path: `/catalog/search/${owner}/${repo}`,
    query: {
      q: q,
      owner: owner,
      repo: repo,
