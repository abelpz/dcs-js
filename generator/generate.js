...
(async () => {
  const source = await (await fetch(sourceUrl)).json();
  const swagger = await parser.bundle(source);
  const utils = {
    camelCase,
    paramCase,
    safeJs,
  ...
  async function createModel(preparedModel) {
    ...
    fs.outputFileSync(
      path.join(rootDir, "models", `${paramCase(preparedModel.name)}.ts`),
      modelFileContent
    );
  }
  ...
  async function createModels(models) {
    const barrel = Object.keys(models)
      ...
      .join("");
    fs.outputFileSync(path.join(rootDir, "models", `index.ts`), barrel);
  }
  ...
  async function createTagFunctions(tag, tagName) {
    ...
    fs.outputFileSync(
      path.join(rootDir, paramCase(tagName), `index.ts`),
      funcBarrel.join("")
    );
  }
  ...
  async function createUtils() {
    ...
    fs.outputFileSync(path.join(rootDir, `utils.ts`), utilsContent);
  }
  ...
  async function createFunctions(data) {
    ...
    fs.outputFileSync(path.join(rootDir, `index.ts`), tagsBarrel.join(""));
  }
